...
const AUTHORING_CONTRACT = [
  ...
  '- The component MUST be the default export. Props: { appState, onUpdateState, appId, backend, userProfile } — e.g. `export default function Foo({ appState, onUpdateState, backend }) { ... }`.',
  ...
  '- window.dashterm.secrets.fetch(url, init?)        — call a 3rd-party API with {{secret.NAME}} placeholders substituted server-side (the value never reaches the browser). window.dashterm.secrets.names() lists available secret names.',
  ...
  '    ctx.secrets.get(name) / ctx.secrets.names()             the owner\'s stored secrets',
  '    ctx.ai.chat(messages, opts?)                            the owner\'s AI provider',
  '    ctx.userId, ctx.log(...)',
